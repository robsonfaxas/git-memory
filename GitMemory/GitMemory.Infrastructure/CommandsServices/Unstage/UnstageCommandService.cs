        ...
        public Task<CommandResponse> ExecuteCommand(List<string> commands)
        {
            try
            {
                if (commands == null || commands.Count == 0)
                    return Task.FromResult(new CommandResponse(ResourceMessages.Services_Unstage_MissingArguments, ResponseTypeEnum.Error));
                    ...
                    foreach(var command in commands)
                    {
                            ...
                            foreach (var staged in repository.Staged.ToList())
                            {
                                if (repository.Staged.Select(p => p.CommitHash.ToLower()).Contains(staged.CommitHash.ToLower()))
                                {
                                    ...
                                    CommandContextAccessor.Current.InteractionWindow
                                        .Write(new CommandResponse(string.Format(ResourceMessages.Services_Unstage_UnstagedCommit, staged.CommitHash)));
                                    totalUnstaged++;
                                }
                            }
                            break;
                        ...
                        else if (repository.Staged.Select(p => p.CommitHash.ToLower()).Contains(command.ToLower()))
                        {
                            ...
                            if (commit is not null)
                            {
                                ...
                                CommandContextAccessor.Current.InteractionWindow
                                    .Write(new CommandResponse(string.Format(ResourceMessages.Services_Unstage_UnstagedCommit, command)));
                                totalUnstaged++;
                            }                               
                        }
                        ...
                        {
                            CommandContextAccessor.Current.InteractionWindow
                                .Write(new CommandResponse(string.Format(ResourceMessages.Services_Unstage_InvalidHash, command), ResponseTypeEnum.Error));
                        }
                    }
                }
                ...
                if (totalUnstaged > 0)
                {
                    return Task.FromResult(new CommandResponse(ResourceMessages.Services_Unstage_Success));
                }
                return Task.FromResult(new CommandResponse(ResourceMessages.Services_Unstage_SuccessZeroCommits));
            }
            catch (Exception ex)
            {
